export default function BriefCard({ image, imageAlt, title, description, hasBorder, borderStyle, hasShadow, customStyle, hasUnderline }) {
  const baseClasses = "flex grow shrink justify-center items-center self-stretch my-auto min-h-[275px] min-w-[240px] w-[220px]";
    ...
    return (
      <div className={`relative ${classes}`}>
        <img
          ...
          src={image}
          alt={imageAlt}
          className={`object-contain ${hasShadow ? customStyle : ''} ${title ? 'absolute top-2/4 left-2/4 z-0 self-start rounded-full -translate-x-2/4 -translate-y-2/4' : ''} aspect-square ${title ? 'h-[250px] min-w-[240px] w-[250px]' : ''}`}
        />
        {title && (
